...
const { searchFilterSortMiddleware } = require('../../middleware/getFunction.middleware');
...
vegetableRoute.get('/',searchFilterSortMiddleware,async(req,res)=>{
  ...
  const vegetables = await VegetableModel.find(query).sort(sort).skip(skip).limit(limit);
